        ...
        chrome.scripting.executeScript(
          {
            ...
            function: () => {
              ...
              const textArray = Array.from(specificElements).map(element => element.innerText);
              return textArray;
            }
          },
          function(results) {
            const textArrayFromElements = results[0];
            console.log('Text from specific elements:', textArrayFromElements);
  
            // Modify the text content of specific elements
            const specificElements = document.querySelectorAll('.single__content___9ekjR .rich-text:not(.sn-conversion) p');
